...
function StepEditForm({ step, onSave, onCancel }: { step: Partial<Step>; onSave: (s: Partial<Step>) => void; onCancel: () => void }) {
  ...
  return (
    <div className="bg-dark-800 border border-blue-200 rounded-xl p-4 space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2">
          <label className="text-xs font-medium text-gray-600 mb-1 block">Step name *</label>
          <input value={form.name || ''} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="w-full border border-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
        </div>
        <div className="col-span-2">
          <label className="text-xs font-medium text-gray-600 mb-1 block">Description</label>
          <textarea value={form.description || ''} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={3} className="w-full border border-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400 resize-none" />
        </div>
        <div>
          <label className="text-xs font-medium text-gray-600 mb-1 block">Team / swim lane</label>
          <input value={form.swim_lane || ''} onChange={e => setForm(f => ({ ...f, swim_lane: e.target.value }))} placeholder="e.g. Security Team" className="w-full border border-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
        </div>
        <div>
          <label className="text-xs font-medium text-gray-600 mb-1 block">Duration (minutes)</label>
          <input type="number" value={form.estimated_duration_minutes || 15} onChange={e => setForm(f => ({ ...f, estimated_duration_minutes: parseInt(e.target.value) || 15 }))} className="w-full border border-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
        </div>
        <div>
          <label className="text-xs font-medium text-gray-600 mb-1 block">Step type</label>
          <select value={form.step_type || 'HUMAN'} onChange={e => setForm(f => ({ ...f, step_type: e.target.value }))} className="w-full border border-gray-600 rounded-lg px-3 py-2 text-sm bg-dark-900 bg-opacity-50 focus:outline-none focus:ring-2 focus:ring-brand-400">
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button onClick={onCancel} className="text-xs border border-gray-600 px-3 py-2 rounded-lg hover:bg-dark-800">Cancel</button>
      </div>
    </div>
  );
}
...
export function SOEStepEditor({ eventId, onClose }: StepEditorModalProps) {
  ...
  return (
    <div style={{ minHeight: 600, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} className="fixed inset-0 z-50" onClick={onClose}>
      <div className="bg-dark-900 bg-opacity-50 rounded-2xl shadow-2xl w-full max-w-3xl mx-4 max-h-[90vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-4 border-t border-gray-700 shrink-0">
          <div className="flex gap-3 items-start">
            <div className="flex-1">
              ...
              <input value={deviationNote} onChange={e => setDeviationNote(e.target.value)} placeholder="Why are you editing this SOE?" className="w-full border border-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-brand-400" />
            </div>
