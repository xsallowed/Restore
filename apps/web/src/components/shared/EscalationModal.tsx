...
const SEVERITIES = [
  { value: 'LOW',      label: 'Low',      color: 'border-gray-300 bg-dark-800 text-gray-300',   desc: 'Minor blocker — can proceed with workaround' },
  ...
  { value: 'CRITICAL', label: 'Critical', color: 'border-red-300 bg-dark-800 text-red-800',         desc: 'Recovery at risk — escalate now' },
];
    ...
    >
      <div className="bg-dark-900 bg-opacity-50 rounded-2xl shadow-2xl w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-4 space-y-4">
            ...
            <div className="space-y-2">
              {SEVERITIES.map(s => (
                <label key={s.value} className={clsx(
                  ...
                  severity === s.value ? s.color + ' border-opacity-100' : 'border-gray-700 hover:border-gray-600'
                )}>
                  <input
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full border border-gray-600 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-300 resize-none"
            />
          </div>
        </div>
        ...
        <div className="px-5 pb-5 flex gap-3">
          <button onClick={onClose} className="flex-1 text-sm font-medium text-gray-600 border border-gray-600 py-2.5 rounded-lg hover:bg-dark-800">
            Cancel
          </button>
          <button
