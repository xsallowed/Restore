...
const STATUS_COLORS: Record<string, string> = { HEALTHY:'bg-green-100 text-green-800', DEGRADED:'bg-yellow-100 text-yellow-800', CRITICAL:'bg-orange-100 text-orange-800', OFFLINE:'bg-red-100 text-red-800', UNKNOWN:'bg-gray-100 text-gray-600' };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-4 grid grid-cols-2 gap-4 max-h-96 overflow-y-auto">
          <div className="col-span-2">
            ...
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. prod-db-primary-01" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Asset type *</label>
            <select value={form.assetType} onChange={e => setForm(f => ({ ...f, assetType: e.target.value }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-400">
          ...
          <div>
            ...
            <select value={form.environment} onChange={e => setForm(f => ({ ...f, environment: e.target.value }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-400">
              {ENVIRONMENTS.map(e => <option key={e} value={e}>{e}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Criticality tier</label>
            <select value={form.criticalityTier} onChange={e => setForm(f => ({ ...f, criticalityTier: parseInt(e.target.value) }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-400">
              {[1,2,3,4].map(t => <option key={t} value={t}>Tier {t}{t===1?' (Critical)':t===4?' (Low)':''}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Owner</label>
            <input value={form.owner} onChange={e => setForm(f => ({ ...f, owner: e.target.value }))} placeholder="Team or person name" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Location</label>
            <input value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} placeholder="e.g. us-east-1, DC-London" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
          </div>
          <div className="col-span-2">
            <label className="block text-xs font-medium text-gray-600 mb-1">Recovery group</label>
            <input value={form.recoveryGroup} onChange={e => setForm(f => ({ ...f, recoveryGroup: e.target.value }))} placeholder="e.g. DATABASE_TIER, NETWORK_CORE" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
          </div>
        </div>
  ...
  return (
    <div style={{ minHeight: 460, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2"><Network size={16} className="text-brand-500" /><h2 className="font-semibold text-gray-900 text-sm">Dependencies — {asset.name}</h2></div>
          <button onClick={onClose}><X size={16} className="text-gray-400" /></button>
        </div>
        <div className="px-5 py-4 space-y-4">
          <div className="grid grid-cols-3 gap-2">
            <div><label className="text-xs text-gray-500 mb-1 block">Source</label>
              <select value={sourceId} onChange={e => setSourceId(e.target.value)} className="w-full border border-gray-200 rounded-lg px-2 py-2 text-xs bg-white focus:outline-none">
            ...
            <div><label className="text-xs text-gray-500 mb-1 block">Relationship</label>
              <select value={relType} onChange={e => setRelType(e.target.value)} className="w-full border border-gray-200 rounded-lg px-2 py-2 text-xs bg-white focus:outline-none">
                {RELATIONSHIP_TYPES.map(r => <option key={r} value={r}>{r}</option>)}
              </select>
            </div>
            <div><label className="text-xs text-gray-500 mb-1 block">Target *</label>
              <select value={targetId} onChange={e => setTargetId(e.target.value)} className="w-full border border-gray-200 rounded-lg px-2 py-2 text-xs bg-white focus:outline-none">
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-gray-900">Technology Asset Registry</h1><p className="text-sm text-gray-500 mt-0.5">{assets.length} assets · FR0.1.1</p></div>
        <div className="flex gap-2">
          <button onClick={() => navigate('/assets/graph')} className="flex items-center gap-1.5 text-sm border border-gray-200 px-3 py-2 rounded-lg hover:bg-gray-50"><Network size={14} /> Graph</button>
          <button onClick={() => setShowCsv(s => !s)} className="flex items-center gap-1.5 text-sm border border-gray-200 px-3 py-2 rounded-lg hover:bg-gray-50"><Upload size={14} /> CSV import</button>
            ...
            <tbody className="divide-y divide-gray-50">
              {assets.map(asset => (
                <tr key={asset.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-xs"><span className="bg-gray-100 text-gray-700 px-1.5 py-0.5 rounded">{asset.environment}</span></td>
                  <td className="px-4 py-3 text-center text-xs font-bold text-gray-700">T{asset.criticality_tier}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      <button onClick={() => setEditAsset(asset)} className="p-1.5 hover:bg-gray-100 rounded text-gray-400 hover:text-gray-700" title="Edit"><Edit2 size={13} /></button>
                      <button onClick={() => setDepAsset(asset)} className="p-1.5 hover:bg-gray-100 rounded text-gray-400 hover:text-blue-500" title="Dependencies"><Link size={13} /></button>
                      <button onClick={() => { if (window.confirm(`Delete ${asset.name}?`)) deleteMutation.mutate(asset.id); }} className="p-1.5 hover:bg-red-50 rounded text-gray-400 hover:text-red-500" title="Delete"><Trash2 size={13} /></button>
                    </div>
                  </td>
                </tr>
              ))}
