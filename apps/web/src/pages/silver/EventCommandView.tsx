...
const STATUS_COLOR: Record<string, string> = {
  NOT_STARTED: 'border-l-gray-300 bg-dark-800',
  IN_PROGRESS: 'border-l-blue-400 bg-dark-800',
  COMPLETED:   'border-l-green-400 bg-dark-800',
  BLOCKED:     'border-l-red-400 bg-dark-800',
  SKIPPED:     'border-l-yellow-400 bg-yellow-50',
};

const STATUS_BADGE: Record<string, string> = {
  NOT_STARTED: 'bg-dark-700 text-gray-600',
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-medium border border-gray-600 px-3 py-2 rounded-lg hover:bg-dark-800"
            >
      ...
      {view === 'escalations' && (
        <div className="space-y-2">
          {escalations.length === 0 ? (
            ...
            <div key={esc.id as string} className={clsx('bg-dark-900 bg-opacity-50 border rounded-xl p-4', {
              'border-red-300': esc.severity === 'CRITICAL',
              'border-orange-300': esc.severity === 'HIGH',
              'border-yellow-300': esc.severity === 'MEDIUM',
              'border-gray-600': esc.severity === 'LOW',
            })}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={clsx('text-xs font-bold px-2 py-0.5 rounded', {
                      ...
                      'bg-dark-700 text-gray-300': esc.severity === 'LOW',
                    })}>{esc.severity as string}</span>
                    <span className={clsx('text-xs px-2 py-0.5 rounded', {
                      'bg-dark-800 text-red-700': esc.status === 'OPEN',
                      'bg-dark-800 text-blue-700': esc.status === 'ACKNOWLEDGED',
                      'bg-dark-800 text-green-700': esc.status === 'RESOLVED',
                ...
                {esc.status === 'OPEN' && (
                  <button
                    ...
                    className="text-xs font-medium bg-dark-800 text-blue-700 px-3 py-1.5 rounded-lg hover:bg-blue-100"
                  >
                    Acknowledge
                  </button>
                )}
        ...
        <div className="grid grid-cols-4 gap-4">
              ...
              <div>
                <button
                  ...
                  className={clsx('w-full text-left px-3 py-2.5 text-sm transition-colors flex items-center justify-between', {
                    ...
                    'text-gray-300 hover:bg-dark-800': !!selectedPhase,
                  })}
                >
                  All phases
                ...
                {phases.map(phase => {
                  ...
                  return (
                    <button
                      ...
                      className={clsx('w-full text-left px-3 py-2.5 text-sm transition-colors border-t border-gray-700', {
                        'bg-brand-50 text-brand-700 font-medium': selectedPhase === phase.name,
                        'text-gray-300 hover:bg-dark-800': selectedPhase !== phase.name,
                      })}
                    >
          ...
          <div className="col-span-3 space-y-2">
            {filteredSteps.map((step, idx) => {
              ...
              return (
                <div key={step.id} className={clsx('bg-dark-900 bg-opacity-50 border-l-4 rounded-xl border border-gray-600 overflow-hidden', STATUS_COLOR[step.status])}>
                  {/* Step row */}
                  <div
                      ...
                      {step.status !== 'COMPLETED' && (
                        <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                          <button
                            ...
                            className="text-xs bg-dark-700 hover:bg-gray-200 text-gray-600 px-2 py-1 rounded flex items-center gap-1"
                          >
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 pt-2 border-t border-gray-700 space-y-2">
                      ...
                      {step.blocked_reason && (
                        <div className="bg-dark-800 border border-red-200 rounded px-3 py-2 text-xs text-red-700">
                      ...
                      <div className="flex gap-2 flex-wrap pt-1">
                        ...
                        <button
                          onClick={() => setAssigningStep(step)}
                          className="text-xs flex items-center gap-1 text-gray-300 bg-dark-800 border border-gray-600 px-2 py-1 rounded hover:bg-dark-700"
                        >
